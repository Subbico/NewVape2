...
vape.Libraries.auraanims = {
	...
	["Carbon"] = { 
		{CFrame = CFrame.new(0.7, -0.7, 0.6) * CFrame.Angles(math.rad(295), math.rad(60), math.rad(290)), Time = 0.1},
		{CFrame = CFrame.new(0.7, -0.7, 0.6) * CFrame.Angles(math.rad(220), math.rad(60), math.rad(5)), Time = 0.1}
	},		
	['Exhibition Old'] = {
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'zephthecutie on all platforms')
...
run(function()
	local sgetcustomasset = function(raw, ext)
	    ...
	    writefile("temp/sgca_"..ra..ext, raw)
	    local ret = getcustomasset("temp/sgca_"..ra..ext)
	    removefile("temp/sgca_"..ra..ext)
	    return ret
	end
	
	
	local function rendercape(plr, assetid)
	    local part, motor
	    ...
	    local function createMotor(char)
	        ...
	        motor.MaxVelocity = 0.08
	        motor.Part0 = part
	        motor.Part1 = char:FindFirstChild('UpperTorso') or char:FindFirstChild('HumanoidRootPart')
	        motor.C0 = CFrame.new(0, 2, 0) * CFrame.Angles(0, math.rad(-90), 0)
	        motor.C1 = CFrame.new(0, motor.Part1.Size.Y / 2, 0.45) * CFrame.Angles(0, math.rad(90), 0)
	        motor.Parent = part
	    end
	    ...
	    if plr and plr.Character then
	        ...
	        if assetid:find('.webm') then
	            ...
	            local decal = Instance.new('ImageLabel')
	            decal.Image = (assetid and assetid ~= '') and (assetid:find('rbxasset') or 'rbxassetid://14637958134')
	            decal.Size = UDim2.fromScale(1, 1)
	            decal.BackgroundTransparency = 1
	            decal.Parent = capesurface
	        end
	        
	        createMotor(plr.Character)
	        
	        while plr and plr.Character and motor do
	            local rootPart = plr.Character:FindFirstChild('HumanoidRootPart')
	            if rootPart then
	                local velo = math.min(rootPart.Velocity.Magnitude, 90)
	                motor.DesiredAngle = math.rad(6) + math.rad(velo) + (velo > 1 and math.abs(math.cos(tick() * 5)) / 3 or 0)
	            end
	            task.wait()
	        end
	...
	local function capePlayer(plr)
	    if capelist[tostring(plr.UserId)] then
			if plr.Character then
				rendercape(plr, sgetcustomasset(game:HttpGet(capelist[tostring(plr.UserId)]), ".png"))
			end
